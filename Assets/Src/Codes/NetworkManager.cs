...
public class NetworkManager : MonoBehaviour
{
        ...
        if (IsValidPort(port)) {
            ...
            if (deviceIdInputField.text != "") {
                ...
                if (GameManager.instance.deviceId == "") {
                    GameManager.instance.deviceId = GenerateUniqueID(); // deviceId가 없으면 새로 생성하여 넣어주기
                }
            }
    ...
    // 공통 패킷 생성 함수
    async void SendPacket<T>(T payload, uint handlerId, Packets.PacketType packetType)
    {
        ...
        // 헤더 생성
        byte[] header = CreatePacketHeader(data.Length, packetType);
    ...
    void SendInitialPacket() {
        ...
        // handlerId는 0으로 가정
        SendPacket(initialPayload, (uint)Packets.HandlerIds.Init, Packets.PacketType.Normal);
    }
    ...
    public void SendLocationUpdatePacket(float x, float y) {
        ...
        SendPacket(locationUpdatePayload, (uint)Packets.HandlerIds.LocationUpdate, Packets.PacketType.Location);
    }
    ...
    void ProcessResponseData(byte[] data) {
        try {
            ...
            SpecificDataType specificData = JsonConvert.DeserializeObject<SpecificDataType>(jsonString);
            initPositionX = specificData.x;
            initPositionY = specificData.y;
            /*SpecificDataType specificData2 = JsonUtility.FromJson<SpecificDataType>(jsonString);
            Debug.Log(specificData2);*/
